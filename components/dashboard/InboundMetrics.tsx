...
async function fetchInbound(): Promise<InboundKpis> {
  const res = await fetch("/api/ringcentral/inbound");
  if (!res.ok) {
    const e = await res.json().catch(() => ({ error: "Unknown error" }));
    throw new Error(e.error ?? "Failed to fetch inbound data");
  }
  return res.json();
}
...
export function InboundMetrics() {
  ...
  const { data, isLoading, isError, error } = useQuery<InboundKpis>({
    queryKey: ["rc-inbound"],
  ...
  return (
    <section>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          onClick={() => queryClient.invalidateQueries({ queryKey: ["rc-inbound"] })}
